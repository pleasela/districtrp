...
Citizen.CreateThread(function()
	...
	moderationNotificationPrivate = GetConvar("ea_moderationNotificationPrivate", "false")
	...
	AddEventHandler('EasyAdmin:kickPlayer', function(playerId,reason)
		if DoesPlayerHavePermission(source,"easyadmin.kick") then
			local character = getIdentity(playerId)
			if character ~= nil then
				SendWebhookMessage(moderationNotification,string.format(strings.adminkickedplayer, GetPlayerName(source), GetPlayerName(playerId), character.firstname .. " " .. character.lastname, reason))
				SendWebhookMessage(moderationNotificationPrivate,string.format(strings.adminkickedplayer, GetPlayerName(source), GetPlayerName(playerId), character.firstname .. " " .. character.lastname, reason))
			else
				...
				SendWebhookMessage(moderationNotificationPrivate,string.format(strings.adminkickedplayer, GetPlayerName(source), GetPlayerName(playerId), '', reason))
			end
	...
	RegisterCommand('rannounce', function(source, args)
	    if source == 0 then
	        TriggerClientEvent('chatMessage', -1, "ANNOUNCEMENT", {255, 0, 0}, "" .. table.concat(args, " "))
	    else
	...
	AddEventHandler('EasyAdmin:banPlayer', function(playerId,reason,expires)
		if DoesPlayerHavePermission(source,"easyadmin.ban") then
			...
			local character = getIdentity(playerId)
			if character ~= nil then
				reason = reason.. string.format(strings.reasonadd, GetPlayerName(playerId), character.firstname .. " " .. character.lastname, GetPlayerName(source) )
			else
			...
			SendWebhookMessage(moderationNotificationPrivate,string.format(strings.adminbannedplayer, GetPlayerName(source), GetPlayerName(playerId), reason, os.date('%d/%m/%Y 	%H:%M:%S', expires ) ))
	...
	RegisterCommand("kick", function(source, args, rawCommand)
		if args[1] and tonumber(args[1]) and DoesPlayerHavePermission(source,"easyadmin.kick") then
			...
			if GetPlayerName(args[1]) then
				local character = getIdentity(args[1])
				if character ~= nil then
					SendWebhookMessage(moderationNotification,string.format(strings.adminkickedplayer, GetPlayerName(source), GetPlayerName(args[1]), character.firstname .. " " .. character.lastname, reason))
					SendWebhookMessage(moderationNotificationPrivate,string.format(strings.adminkickedplayer, GetPlayerName(source), GetPlayerName(args[1]), character.firstname .. " " .. character.lastname, reason))
				else
					...
					SendWebhookMessage(moderationNotificationPrivate,string.format(strings.adminkickedplayer, GetPlayerName(source), GetPlayerName(args[1]), '', reason))
				end
	...
	RegisterCommand("ban", function(source, args, rawCommand)
		if args[1] and tonumber(args[1]) and source == 0 then
			local reason = ""
			for i,theArg in pairs(args) do
				if i ~= 1 then
					reason = reason.." "..theArg
			...
			if GetPlayerName(args[1]) then
				...
				local character = getIdentity(args[1])
				if character ~= nil then
					reason = reason.. string.format(strings.reasonadd, GetPlayerName(args[1]), character.firstname .. " " .. character.lastname, "Administrator")
				else
					reason = reason.. string.format(strings.reasonadd, GetPlayerName(args[1]), '', "Administrator")
				end
				...
				updateBlacklist( ban )
				SendWebhookMessage(moderationNotification,string.format(strings.adminbannedplayer, "Administrator", GetPlayerName(args[1]), reason, os.date('%d/%m/%Y 	%H:%M:%S', expires)))
				SendWebhookMessage(moderationNotificationPrivate,string.format(strings.adminbannedplayer, "Administrator", GetPlayerName(args[1]), reason, os.date('%d/%m/%Y 	%H:%M:%S', expires)))
				DropPlayer(args[1], string.format(strings.banned, reason, os.date('%d/%m/%Y 	%H:%M:%S', 10444633200 ) ) )
			else
				print(strings.playernotfound)
			end
		else
			if args[1] and tonumber(args[1]) and DoesPlayerHavePermission(source,"easyadmin.ban") then
				...
				if GetPlayerName(args[1]) then
					...
					local character = getIdentity(args[1])
					if character ~= nil then
						reason = reason.. string.format(strings.reasonadd, GetPlayerName(args[1]), character.firstname .. " " .. character.lastname, GetPlayerName(source))
					else
					...
					SendWebhookMessage(moderationNotificationPrivate,string.format(strings.adminbannedplayer, GetPlayerName(source), GetPlayerName(args[1]), reason, os.date('%d/%m/%Y 	%H:%M:%S', expires)))
					DropPlayer(args[1], string.format(strings.banned, reason, os.date('%d/%m/%Y 	%H:%M:%S', 10444633200 ) ) )
				else
					TriggerClientEvent("chat:addMessage", source, { args = { "EasyAdmin", strings.playernotfound } })
				end
	...
	RegisterCommand("slap", function(source, args, rawCommand)
		if args[1] and args[2] and DoesPlayerHavePermission(source,"easyadmin.slap") then
			SendWebhookMessage(moderationNotificationPrivate,string.format(strings.adminslappedplayer, GetPlayerName(source), GetPlayerName(args[1]), args[2]))
	...
	AddEventHandler('EasyAdmin:SlapPlayer', function(playerId,slapAmount)
		if DoesPlayerHavePermission(source,"easyadmin.slap") then
			SendWebhookMessage(moderationNotificationPrivate,string.format(strings.adminslappedplayer, GetPlayerName(source), GetPlayerName(playerId), slapAmount))
	...
	AddEventHandler('EasyAdmin:FreezePlayer', function(playerId,toggle)
		if DoesPlayerHavePermission(source,"easyadmin.freeze") then
			if toggle then
				SendWebhookMessage(moderationNotificationPrivate,string.format(strings.adminfrozeplayer, GetPlayerName(source), GetPlayerName(playerId)))
			else
				SendWebhookMessage(moderationNotificationPrivate,string.format(strings.adminunfrozeplayer, GetPlayerName(source), GetPlayerName(playerId)))
			end
