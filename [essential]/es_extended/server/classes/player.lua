function CreateExtendedPlayer(player, accounts, inventory, job, loadout, name, lastPosition)
  ...
  self.goOffDuty = function()
	if not self.job['extra'] or self.job['extra'] == "unemployed" then
		self.job['extra'] = self.job['name']
		self.setJob("unemployed", self.job['grade'])
	end
  end
  self.goOnDuty = function()
	if self.job['name'] == "unemployed" then
		self.setJob(self.job['extra'], self.job['grade'])
		self.job['extra'] = "unemployed"
	end
  end
    ...
    MySQL.Async.fetchAll(
      ...
      function(result)
        ...
        MySQL.Async.fetchAll(
          ...
          {
            ['@job_name'] = name,
            ['@grade']    = (name=="unemployed") and 0 or grade
          },
          function(result)
