...
function startLoopAnim(lib, anim)
	Citizen.CreateThread(function()
		...
		TaskPlayAnim(GetPlayerPed(-1), lib ,anim ,8.0, -8.0, -1, 1, 0, false, false, false)
	end)
end

function startLoopControlAnim(lib, anim)
	Citizen.CreateThread(function()
		RequestAnimDict(lib)
		while not HasAnimDictLoaded( lib) do
			Citizen.Wait(1)
		end
		TaskPlayAnim(GetPlayerPed(-1), lib ,anim ,8.0, -8.0, -1, 33, 0, false, false, false)
	end)
end
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'animations_sub',
		{
			...
			if type == 'scenario' then
				startScenario(anim)
			elseif type == 'animLoop' then
				startLoopAnim(lib, anim)
			elseif type == 'animLoopControl' then
				startLoopControlAnim(lib, anim)
			else
