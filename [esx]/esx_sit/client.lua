...
Citizen.CreateThread(function()
	while true do
		...
		if (GetLastInputMethod(0) and IsControlJustPressed(1, 38) and IsControlPressed(1, 21)) and not IsPedInAnyVehicle(playerPed, true) then			
...
function sit(object, modelName, data)
	...
	ESX.TriggerServerCallback('esx_interact:getPlace', function(occupied)
		...
		if occupied then
			ESX.ShowNotification("This place is taken...")
		else
			local playerPed = GetPlayerPed(-1)
