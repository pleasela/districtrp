  ...
  if PhoneNumbers[phoneNumber] ~= nil then
    ...
    for k,v in pairs(PhoneNumbers[phoneNumber].sources) do
      ...
      if numHasDispatch then
        ...
        local senderName = identity.firstname .. " " .. identity.lastname
        TriggerClientEvent('esx_phone:onMessage', numSource, xPlayer.get('phoneNumber'), message, numPosition, (numHide and true or anon), numType, GetDistpatchRequestId(), source, senderName, phoneNumber)
      else
        TriggerClientEvent('esx_phone:onMessage', numSource, xPlayer.get('phoneNumber'), message, numPosition, (numHide and true or anon), numType, false, source, senderName, phoneNumber)
      end
...
AddEventHandler('esx_phone:stopDispatch', function(dispatchRequestId, senderSource)
  TriggerClientEvent('esx_phone:stopDispatch', -1, dispatchRequestId, GetPlayerName(source))
  TriggerClientEvent("pNotify:SendNotification", senderSource, {
            text = "Your message has gotten through and is acknowledged",
            type = "success",
            queue = "dispatch",
            timeout = 10000,
            layout = "centerRight"
...
ESX.RegisterServerCallback('esx_phone:checkCash', function(source, cb, senderSource, phoneNumber)
	local xPlayer = ESX.GetPlayerFromId(senderSource)

	if mecanoCallPrice > 0 and xPlayer.get('money') >= mecanoCallPrice then
	    cb(true)
	else
      TriggerClientEvent('esx:showNotification', senderSource, "Invalid amount of money, message cancelled.")
	    cb(false)
	end
end)


RegisterServerEvent('esx_phone:doBooking')
AddEventHandler('esx_phone:doBooking', function(senderSource, phoneNumber)
	local xPlayer = ESX.GetPlayerFromId(senderSource)

	if mecanoCallPrice > 0 and xPlayer.get('money') >= mecanoCallPrice then
	    TriggerEvent('esx_addonaccount:getSharedAccount', "society_" .. phoneNumber, function(account)
	      xPlayer.removeMoney(mecanoCallPrice)
	      account.addMoney(mecanoCallPrice)
	    end)

	    TriggerClientEvent('esx:showNotification', xPlayer.source, 'You have been charged ' .. mecanoCallPrice .. " for a booking fee.")
	else
	    TriggerClientEvent('esx:showNotification', xPlayer.source, "Invalid amount of money, message cancelled.")
	    return
	end
end)