...
AddEventHandler('esx_phone:onMessage', function(phoneNumber, message, position, anon, job, dispatchRequestId, source, senderName, targetPhoneNumber)
	if job == 'player' then
		ESX.ShowNotification(_U('new_message', message))
	else
		if anon == false and senderName ~= nil then
			ESX.ShowNotification('~b~' .. job .. ': ~y~[' .. senderName .. "] ~s~" .. message)
		else
...
RegisterNetEvent('esx_phone:open')
AddEventHandler('esx_phone:open', function()	
  if not ESX.UI.Menu.IsOpen('phone', GetCurrentResourceName(), 'main') then
		ESX.UI.Menu.CloseAll()
		ESX.UI.Menu.Open('phone', GetCurrentResourceName(), 'main')
	end										
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0, Keys['E']) and GetLastInputMethod(2) and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'dispatch' then
					if (CurrentActionData.targetPhoneNumber == 'mecano') then
						ESX.TriggerServerCallback("esx_phone:checkCash", function(hasMoney)
							print("hasmoneuy")
							print(hasMoney)
							if hasMoney then
								TriggerServerEvent('esx_phone:doBooking', CurrentActionData.senderSource, CurrentActionData.targetPhoneNumber)
								TriggerServerEvent('esx_phone:stopDispatch', CurrentDispatchRequestId, CurrentActionData.senderSource)
								SetNewWaypoint(CurrentActionData.position.x,  CurrentActionData.position.y)
							else
								ESX.ShowNotification('The client does not have enough to pay for the booking fees.')
							end
                		end, CurrentActionData.senderSource, CurrentActionData.phoneNumber)
					else
						TriggerServerEvent('esx_phone:stopDispatch', CurrentDispatchRequestId, CurrentActionData.senderSource)
						SetNewWaypoint(CurrentActionData.position.x,  CurrentActionData.position.y)
					end
