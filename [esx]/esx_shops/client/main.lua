...
function OpenShopMenu(zone)
	...
	local elements = {}
	local legal = Config.Zones[zone].legal
	...
	for i=1, #Config.Zones[zone].Items, 1 do
		...
		table.insert(elements, {
			label     = item.name .. ' - <span style="color:green;">$' .. item.price .. ' </span>',
			value     = item.name,
			price     = item.price
		})
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			if zone == 'ExtraBlackShop' then
				TriggerServerEvent('esx_shop:buyIllegalItem', data.current.value, data.current.price)
			else
...
Citizen.CreateThread(function()
	for k,v in pairs(Config.Zones) do
	  	for i = 1, #v.Pos, 1 do
	  		if not (v.legal ~= nil and v.legal == false) then
