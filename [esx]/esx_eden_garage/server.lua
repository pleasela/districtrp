...
ESX.RegisterServerCallback('eden_garage:getVehicles', function(source, cb)
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier LIMIT 38",{['@identifier'] = xPlayer.identifier}, function(data)
...
--Stock les véhicules
ESX.RegisterServerCallback('eden_garage:storev',function(source,cb, vehicleProps)
	local isFound = false
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	...
	--print(plate)
		...
		for _,v in pairs(vehicules) do
			if(plate == v.plate) then
				...
				MySQL.Sync.execute("UPDATE owned_vehicles SET vehicle = @vehprop WHERE id=@id AND vehicle LIKE @model",
				{
					['@vehprop'] = vehprop,
					['@id'] = v.id,
					['@model'] = '"model":' .. model
...
AddEventHandler('eden_garage:modifystate', function(vehicleProps, state)
	...
	for _,v in pairs(vehicules) do
		if(plate == v.plate) then
			MySQL.Sync.execute("UPDATE owned_vehicles SET state=@state WHERE id=@id",
				{
					['@state'] = state,
					['@id'] = v.id,
					['@model'] = '"model":' .. model
...
ESX.RegisterServerCallback('eden_garage:getOutVehicles',function(source, cb)
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier AND state=false",{['@identifier'] = xPlayer.identifier}, function(data)
		for _,v in pairs(data) do
			local vehicle = json.decode(v.vehicle)
...
ESX.RegisterServerCallback('eden_garage:checkMoney', function(source, cb, price)
...
AddEventHandler('eden_garage:pay', function(price)
