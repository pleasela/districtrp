		...
		ESX.TriggerServerCallback('esx_truck:checkvehicle',function(valid)
			if (not valid) then
			  ...
			  if vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
					...
					if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
					  ...
					  if locked == 1 or class == 15 or class == 16 or class == 14 then
						  ...
						  TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
						   ...
						   ESX.ShowNotification('This chest is closed')
					  end
					end
				else
				...
				TriggerEvent('esx:showNotification', "Someone already looks at the chest.")
			end
		end, globalplate)
	end
...
Citizen.CreateThread(function()
  while true do
    ...
    Wait(0)
    if IsControlPressed(0, Keys["-"]) and (GetGameTimer() - GUI.Time) > 1000 then     --this is the funiction to change the key to open the menu. 
		if count == 0 then
			openmenuvehicle()
			count = count +1
		else
		...
		for i=1, #weaponList, 1 do
		  ...
		  if HasPedGotWeapon(playerPed,  weaponHash,  false) and weaponList[i].name ~= 'WEAPON_UNARMED' then
			...
			ESX.UI.Menu.Open(
			  ...
			  {
			    title    = 'Content of the inventory',
			    align    = 'bottom-right',
			    elements = elem,
            ...
            if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0  then
              ...
              --  VehicleMaxSpeed(closecar,totalweight,Config.VehicleLimit[GetVehicleClass(closecar)])
                ...
                ESX.ShowNotification('Weight of the chest:'.. Kgweight .. ' Kg / '..MaxVh..' Kg')
				Citizen.Wait(500)
				TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
              else
                ESX.ShowNotification('You have reached the limit of '..MaxVh..' Kg')
              end
			else
				ESX.ShowNotification('Amount invaild')
			end
          ...
          --fin test
            ...
            if not max then
              ...
              ESX.ShowNotification('You cannot carry any more of it')
            end
			    else
			      ESX.ShowNotification('Amount invaild')
			    end
