...
RegisterNetEvent("realisticVehicles:takeDamage")
AddEventHandler("realisticVehicles:takeDamage", function(damage)
	if damage >= 45 then
		TriggerEvent('ko_client:knockout')
	end
	ApplyDamageToPed(GetPlayerPed(-1), damage)
	--print("Get wrecked passenger, took damage: " .. damage)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			local forw = GetEntityForwardVector(vehicle)
			local norm = GetCollisionNormalOfLastHitForEntity(veh)
			lastNorm = (norm.x*forw.x + norm.y*forw.y)
			if lastNorm < -0.6 then
				--Make sure the other thread has time to use this value
				Citizen.Wait(160)
			end
		end
	end
		...
		if isPedDrivingAVehicle() then
			...
			if pedInSameVehicleLast == true then
				...
				if healthEngineCurrent ~= 1000.0 or healthBodyCurrent ~= 1000.0 or healthPetrolTankCurrent ~= 1000.0 then
					...
					if healthEngineCombinedDelta > 3 then
						--Reduce damage for reverse a bit.
						if lastNorm < -0.6 or (lastNorm == 0.0 and math.abs(lastYSpeed) < 2) then
							healthEngineCombinedDelta = healthEngineCombinedDelta * 0.3
						end
					end
					lastYSpeed = ySpeed
					...
					local pedDamage = healthEngineCombinedDelta*0.25
					
					--if GetEntitySpeed(vehicle) > 20 then
						if(pedDamage > 3) then
							--print("Car DeformMult: "..GetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fDeformationDamageMult'))
							--print("You took " .. pedDamage .. " damage from the vehicle.")
							if (pedDamage >= 45) then
								TriggerEvent('ko_client:knockout')
								pedDamage = 45
							end
							ApplyDamageToPed(ped, math.floor(pedDamage))
							for i = 0, 5 do
								local pass = NetworkGetPlayerIndexFromPed(GetPedInVehicleSeat(vehicle,i))
								if pass and pass >= 0 then
									TriggerServerEvent("realisticVehicles:damageOthers",GetPlayerServerId(pass),math.floor(pedDamage))
								end
							end
						end
					--end
