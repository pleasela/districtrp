...
if Faketimer >= 3 then
    ...
    while true do
        ...
        if IsControlPressed(0, 10) and not IsControlPressed(0, 15) --[[ INPUT_PHONE ]] then
            if not listOn then
                local players = {}
                ptable = GetPlayers()
                for _, i in ipairs(ptable) do
                    ...
                    -- local identity = TriggerServerEvent("scoreboard:getIdentity", GetPlayerServerId(i))
                    -- table.insert(players, 
                    -- '<tr style=\"color: rgb(' .. 255 .. ', ' .. 255 .. ', ' .. 255 .. ')\"><td>' .. GetPlayerServerId(i) .. '</td><td>' .. identity.firstname .. " " .. identity.lastname .. '</td></tr>'
                    -- )
                    -- table.insert(players, 
